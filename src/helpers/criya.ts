...
class Criya {
    ...
    states:criya_States; pseudoStates:criya_States; 
    ...
    */
    state<T>(stateName:string, initialValue:T, autoRender=true):[(() => T), ((newVal: T|criya_Func<T, T>) => void)] {
        this.states[stateName] = initialValue;

        const setState = (newVal:T|criya_Func<T, T>) => {
            let stateValue:T;
            //@ts-ignore
            ...
            validSubs.forEach(sub => {
                ...
                if (autoRender) sub.subscriber.render()
            });
        }
